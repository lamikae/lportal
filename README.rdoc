...
The development version in master contains partial MySQL solution, with model alterations that have broken PostgreSQL as well, so the absolute HEAD is not ready. Use the branch 1.0 (still hosted on Rubyforge git) until author imports the branch.
